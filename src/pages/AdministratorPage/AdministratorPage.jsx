...
function AdministratorPage() {
    ...
    const [bottomsId, setBottomsId] = useState(localStorage.getItem('bottomsId') || '');
    const [dressAndSetId, setDressAndSetId] = useState(localStorage.getItem('dressAndSetId') || '');
    const [outerwearId, setOuterwearId] = useState(localStorage.getItem('outerwearId') || '');
    const [shoesId, setShoesId] = useState(localStorage.getItem('shoesId') || '');
    const [accessoriesId, setAccessoriesId] = useState(localStorage.getItem('accessoriesId') || '');
    ...
    const handleInventorySubmit = async () => {
        try {
            const response = await axios.post(`http://localhost:8080/api/v1/inventory/new`,{
                productId,
                colorId,
                categoryId,
                size,
    ...
    const handleColorSubmit = async () => {
        try {
            const response = await axios.post(`http://localhost:8080/api/v1/color/new`,{
                color
            });
            console.log(response.data);
        } catch (error) {
                ...
                return (
                    <>
                        <div className="input-section">
                            ...
                            <select value={parentCategoryName} onChange={(e) => setParentCategoryName(e.target.value)}>
                        ...
                        <div className="input-section">
                            ...
                            <select value={childCategoryName} onChange={(e) => setChildCategoryName(e.target.value)}>
    ...
    return (
        <div className="administrator-page">
            <div className="admin-menu">
                <button onClick={() => setActiveMenu('categoryCreate')}>카테고리 생성</button>
                <button onClick={() => setActiveMenu('productCreate')}>상품 생성</button>
                <button onClick={() => setActiveMenu('inventoryCreate')}>인벤토리 생성</button>
            </div>
            <div className="input-section">
                {renderMenu()}
            </div>
