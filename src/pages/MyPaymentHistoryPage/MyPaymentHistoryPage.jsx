    ...
    return (
        <div className="my-payment-history-page">
            <div className="payhis-page-title">
                <span>나의 구매 기록</span>
                <div className="total-payhis-count">({`${paymentHistory.length}개의 구매 기록`})</div>
            </div>
            ...
            {paymentHistory.map((payment) => (
                <div key={payment.paymentId} className="payhis-container">
                    <div className="payhis-product-info-container">
                        ...
                        <span>{payment.review ? '내가 작성한 리뷰' : '구매 리뷰 남기기'}</span> {/* 리뷰 여부 */} {/* 구매 날짜 */}
                    </div>
                    <div className="payhis-info-container">
                        <span>{new Date(payment.payedAte).toLocaleDateString()}</span> {/* 결제 날짜 */}
                    </div>
                </div>
            ))}
        </div>
