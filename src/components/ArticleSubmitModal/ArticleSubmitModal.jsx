...
function ArticleSubmitModal({ modalType, modalClose }) {
    ...
    const handleInquirySubmit = async () => {
        await axios.post(`http://localhost:8080/api/v1/inquiry/new/${productId}`, {
            memberId: "1",
            name: writer,
            email: email,
                ...
                <div className="review-writing-modal">
                    <div className="modal-top-section">
                        ...
                        <div className="review-writing-modal-product-info">
                            <img src="https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?q=80&w=2124&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" alt="" className='review-writing-modal-img-info'/>
                            <span>상품 이름</span>
                        </div>
                        <MdClose onClick={modalClose} className="close-review-writing-modal"/>
                    </div>
                    <div className="modal-content-section">
                        ...
                        <div className="additional-info">
                            <span>추가 정보</span>
                            <span>해당 정보는 다른 구매자가 제품을 선택 및 구매하는데 도움이 됩니다.</span>
                    ...
                    <div className="modal-content-section">
                        <div className="writer-info">
                            ...
                            <div className="input-cover">
                                <input type="text" value={writer} onChange={(e) => setWriter(e.target.value)} placeholder="ex) 홍길동"/>
                            </div>
                            <span className="last-label">이메일</span>
                            <div className="input-cover">
                                <input type="text" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ex) gildong@gmail.com"/>
                            </div>
                        </div>
                        ...
                        <div className="inquiry-category-select">
                            ...
                            <div className="category-button-box">
                                <button onClick={() => handleCategoryBtnClick('PRODUCT')} className={inquiryCategory === 'PRODUCT' ? 'active' : ''}>상품 문의 글</button>
                                <button onClick={() => handleCategoryBtnClick('DELIVERY')} className={inquiryCategory === 'DELIVERY' ? 'active' : ''}>배송 문의 글</button>
                                <button onClick={() => handleCategoryBtnClick('EXCHANGE')} className={inquiryCategory === 'EXCHANGE' ? 'active' : ''}>교환 문의 글</button>
                                <button onClick={() => handleCategoryBtnClick('RETURN')} className={inquiryCategory === 'RETURN' ? 'active' : ''}>반품 문의 글</button>
                                <button onClick={() => handleCategoryBtnClick('REFUND')} className={inquiryCategory === 'REFUND' ? 'active' : ''}>취소 문의 글</button>
                                <button onClick={() => handleCategoryBtnClick('ETC')} className={inquiryCategory === 'ETC' ? 'active' : ''}>기타 문의 글</button>
                        ...
                        <div className="additional-info">
                            ...
                            <div className="input-cover">
                                <input type="password" value={inquiryPassword} onChange={(e) => setInquiryPassword(e.target.value)} />
                            </div>
                            <span className="last-label">해당 정보는 구매자의 프라이버시를 지키는데 사용됩니다.</span>
                        </div>
